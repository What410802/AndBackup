...
报告现在把它们拆开说：`无记录 {skip}（目录/链接等 {other}，普通文件缺记录 {nosum}）`。
...
| 连记录一起删掉整个成员（文件消失） | **不失败**，只是少一个“通过” | 归档里没有“应有成员清单”，流式 tar 无法在开头声明未来成员 |
| 新插一个自带正确记录的成员 | **不失败**，会多一个“通过” | 记录随成员走，归档无整体签名/清单 |
| 新插一个不带记录的成员 | 一般情况下**不失败**，只多一个“普通文件缺记录” | 同上；只有“全都没记录”才失败 |
| 改名 / 移动成员 | **不失败** | 哈希只盖内容，不盖名字与位置 |

即：这层校验能证明“里面的文件就是打包时读到的那些字节”，不能证明“打包时读到的文件一个都不少、
一个都不多”。要覆盖后一半，需要在归档里再加一份**清单成员**（例如在末尾追加一个
`PAXCK.manifest` 成员，逐条列出成员路径/类型/大小/mtime 与哈希，并给它自己也写上
`PAXCK.checksum.sha256`），再由 `verify` 在见到该成员时双向比对（清单里有没有缺失的、
归档里有没有单外的），见不到清单时退回当前行为并提示。当前尚未实现。
