...
校验分两层：**逐文件记录**（内容）与**末尾成员清单**（成员集合）。

**“无记录”不是归档里存的一个数字**：归档里存的只是每个普通文件自己的 PAX 扩展头
`PAXCK.checksum.sha256`（内容哈希），以及 tar 自带的名称/类型/大小/mtime。`verify` 是
**边读边数**：把 tar 读出来的每个成员都算进 `total`（PAX 扩展头被 `tarfile` 折进
`member.pax_headers`，不单独计一），普通文件按其自己的记录判“通过/失败”，剩下的一律
计进“无记录”列——所以**目录、符号链接等非普通文件与“没有记录的普通文件”本来就在同一列**。
报告把它们拆开说：`无记录 {skip}（目录/链接等 {other}，普通文件缺记录 {nosum}）`。
`nosum` 同时是下面那道闸门的依据：**所有**普通文件都没记录（典型情况：归档根本不是
`paxck create` 生成的，或 pax 头被剥掉）时直接判失败，否则“无记录”只意味着这些成员
没有被校验。`--packed-manifest` 是给主控/`--prune-source` 的**旁路清单**，不写进归档，
因此校验时不看它。

成员集合由**归档末尾的清单成员 `PAXCK.manifest`** 保护：打包器把每个成员的
类型/模式/大小/mtime/uid/gid 与路径（符号链接与硬链接还有目标），按 NUL 分帧写在归档
最后一个成员里，所以路径里的制表符或换行都不会破坏记录。这个成员本身是**普通文件**，
并且带着它自己的 `PAXCK.checksum.sha256`——这就是“自身也带记录”的意思：它和别的文件一样
受逐文件校验保护，改它的内容会被它自己的哈希抓到（但它无法把自己列进自己，所以它不在
自己描述的成员集合里，也不计入 `total`）。

`verify` 与 `extract` 都会**双向比对**：清单里有而归档里没有（缺失）、归档里有而清单里
没有（多余）、同名但类型/模式/大小/mtime/uid/gid/链接目标不同（不一致）、同一路径出现
两次（重复）都判失败。默认**要求清单存在**：旧归档（本版本之前生成的）会因此直接失败，
提示用 `--allow-missing-inventory` 放行——那等于放弃成员集合保护，只剩内容层。

**它不是隐藏成员**：清单在归档根下就是一个普通成员（名称 `PAXCK.manifest`，模式 `0644`、
`mtime` 固定为 0 以保证同样输入得到同样字节），所以 7-Zip、WinRAR、资源管理器、GNU tar
等任何 tar 工具都会把它列出来、解压时会写出一个同名文件。这是刻意的：只有普通文件成员
才能携带自己的 PAX 记录（即上一段说的“自身也带记录”）。本工具自己的两种提取模式
（默认与 `--direct-tarfile`）都把它当记账、**不写入目标目录**；第三方工具会写出它，
删掉它也只影响那些工具的“成员集合”判断，不影响归档内容（本工具校验时会报“没有清单”）。
如果你需要归档里完全不带这个成员（放弃成员集合保护、且校验需加 `--allow-missing-inventory`），
可以提，这个开关还没做。

| 篡改方式 | 结果 | 靠哪一层 |
|---|---|---|
| 改某个文件的内容（保留原记录） | **失败** `SHA-256 不符` | 逐文件哈希 |
| 改归档字节（压缩流 / tar 结构） | **失败** 无法解析或流中断 | xz/gzip 自带的 CRC + tar 结构校验 |
| 截断（少尾） | **失败** | 同上（压缩流尾部校验） |
| 连记录一起删掉整个成员（文件消失） | **失败** `清单里有但归档中缺失的成员` | 成员清单 |
| 新插一个成员（无论有没有记录） | **失败** `归档里有但清单中没有的成员` | 成员清单 |
| 改成员的元信息/类型/链接目标（不动内容） | **失败** `清单与归档的同名成员不一致` | 成员清单 |
| 同一个路径出现两次 | **失败** 重复路径 | 成员清单 |
| 去掉某个文件的记录但留着文件 | 多数情况**不失败**，只多一个“普通文件缺记录” | ——（仅当**所有**普通文件都没记录时才失败） |
| 删掉清单成员本身 | **失败** `归档没有 PAXCK.manifest 成员` | 默认要求清单 |
| 改写清单内容 | **失败** `成员清单自身的 SHA-256 不符` | 逐文件哈希（清单自己也是普通文件） |
| 把整个归档（内容、记录、清单）重建一遍 | **不失败**（内部自洽） | 无——没有签名 |

所以现在能证明的是“**成员一个不少、一个不多，且每个成员的内容就是打包时读到的那些字节**”，
仍**不能**证明“这个归档出自某次真实的打包”。归档里没有密钥、没有签名，任何能改写整个归档
的人都可以让所有层自洽；要防这一类，需要在归档外另存签名（例如对最终文件做 HMAC 或
GPG 签名），不属于当前范围。另一个诚实的代价：清单把成员路径完整写进归档（每个成员约
`路径长度 + 60` 字节，压缩后更小），如果这属于敏感信息，请把归档与备份一起妥善保管；
`--allow-missing-inventory` 只影响校验，不改变归档内容。
