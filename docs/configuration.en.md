...
| `paxck.py` | verifier | `paxck.py verify [ARCHIVE]` or `-i ARCHIVE`, optional `-q`, `--allow-missing-inventory` | auto-detect tar/xz/gzip/zstd; verify each PAX SHA-256 **and** compare the archive with its trailing `PAXCK.manifest` inventory. How the "without a record" column is derived, and the whole protection boundary (content plus set membership, no signature), is in [docs/flow.en.md](flow.en.md#verification-semantics-and-limits) |
| `paxck.py` | extractor | `paxck.py extract [ARCHIVE] -C DEST`, optional `--allow-missing-inventory` | default: per-file verification plus the inventory comparison, then an atomic publish of the staging directory; `DEST` must not exist. `--direct-tarfile` is the trusted-archive direct mode |
...
| `backup.py` | verify | `backup.py verify [ARCHIVE]` or `-i ARCHIVE` | re-verify an existing archive with the same code the backup pipeline uses on the archive it just produced. Purely local: no device, no `--config` (nothing to read), and nothing is written or changed; the exit code is 1 when any record fails. The member inventory is required by default; pass `--allow-missing-inventory` for an older archive |
| `backup.py` | extract | `backup.py extract ARCHIVE -C DIR [--direct-tarfile]` | recover an archive into a **non-existent** `DIR`: per-file verification plus the inventory comparison, published atomically only when everything passes (the staging directory is removed otherwise). `--direct-tarfile` is the tarfile-direct mode for trusted archives (no verification, not atomic) |
