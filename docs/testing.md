...
| `tests/test_sourcetree_unit.py` | 单元 | `tree` 的记录与流式解析：7 字段/5 字段记录、路径中含 `|` 或制表符、垃圾行被拒绝；`_consume_stream` 的换行与 NUL 两种分帧、缺失状态尾标、远端 stderr 收集；`DeviceListing` 对设备端 Python 记录的还原（元数据+路径配对、bytes/str 输入、名字含换行/`|`/制表符、进度行、`\x02U` 不可读条目、`\x03L` 链接目标、垃圾记录只丢不致命）；由 `stat -c %y` 推导 POSIX `TZ`（符号翻转、半小时时区、不可用应答）；`_TreeProgress` 的 `quiet`/`error` 静默、按间隔节流、文案与标签 |
| `tests/test_backup_sh_integration.py` | 离线集成 | 真实 `.sh` + 替身 ADB；`backup.py -> adb_source.py -> paxck.py` 组合、USB serial（不 connect）与 TCP serial（connect）、中文/空格/单引号路径、二进制安全、截断条目只 WARN 跳过且照常发布、`--prune-source`（无权限条目与源根目录保留、`--prune-dry-run` 不删除）、错误退出、`tree`（含符号链接目标与 `--tree-out`、默认走设备端一次性遍历、逐条模式输出与之一致）、旧的 `--list-tree`/`--clean-*` 报错并提示新写法、`clean host-cache`、Android/data 路径、输出目标预检（路径某段是文件、父目录不可写时报错且不产生任何 adb 调用） |
| `tests/test_backup_bat_integration.py` | Windows 离线集成 | 真实 `cmd.exe` + `.bat` + `.cmd` 替身 ADB；`backup.py -> adb_source.py -> paxck.py` 组合、USB serial（不 connect）与 TCP serial（connect）、UTF-8 路径、二进制重定向、gzip/裸 tar、失败清理、命令行 YAML 路径、`tree`（默认设备端 Python 一次列举（环境未部署时用设备端一次性遍历）、
`--tree-mode per-entry`、设备端 Python 模式正文与一次性模式逐字节一致、环境部署后
`auto` 优先用设备端 Python、假设备不支持一次性时回退并告警、未知模式报错）、`--prune-source`（只删已打包条目、跳过的保留、演练、`--prune-dry-run` 需与 `--prune-source` 同用、device-python 走远端清单）、`clean env`/`clean host-cache`/`clean`（缺省 all）、`--clean-env`/`--clean-host-cache` 作为备份收尾（`backup --clean-host-cache` 在成功备份后清理）、旧的 `--list-tree`/`--clean-*` 报错并提示新写法、`device-python` 上传/回读/清理、缺少 `DEVICE_PYTHON` 报错与不自动回退，以及输出目标预检（只读的已存在目标、路径某段是文件时立即失败，不产生任何 adb 调用且原文件字节不变）与已存在目标的处理（无 `-f` 保留原文件并报错退出，带 `-f` 覆写且归档可校验） |
...
格式返回 `%u`/`%g`（Windows 替身用 `st_uid`/`st_gid`，通常为 0），并模拟设备端的
`find -exec stat … {} +` 批量遍历、`find -type l -exec sh -c …` 链接目标收集，以及上传后的
`tree_device.py`（直接跑真脚本、再把记录里的本机路径换回设备路径），同时应答时区探测
`stat -c %y /`。测试断言模式位、属主/组、缩进层级、符号链接目标、`--tree-out` 写文件、
不产生归档文件，以及 `# listing: device-python|oneshot|per-entry` 与实际走的路径一致：
默认（`auto`）走一次性遍历，设备端 Python 模式的输出正文与一次性模式完全一致，
环境已部署后 `auto` 会优先用设备端 Python，Windows 替身用 `FAKE_ADB_NO_ONESHOT=1`
模拟“设备不支持”并断言 `[WARN]` + 回退逐条，POSIX 侧再断言逐条模式与一次性模式的输出
完全一致。`tests/test_sourcetree_unit.py` 另外单独盖住记录分帧与进度节流，不需要 adb。
