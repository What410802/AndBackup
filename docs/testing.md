...
| `tests/test_backup_sh_integration.py` | 离线集成 | 真实 `.sh` + 替身 ADB；`backup.py -> adb_source.py -> paxck.py` 组合、USB serial（不 connect）与 TCP serial（connect）、中文/空格/单引号路径、二进制安全、截断条目只 WARN 跳过且照常发布、`--prune-source`（无权限条目与源根目录保留、`--prune-dry-run` 不删除）、错误退出、`tree`（含符号链接目标与 `--tree-out`、默认走设备端一次性遍历、逐条模式输出与之一致）、`verify`（重新校验成功归档时不产生任何 adb 调用且不改动归档、被改动过的归档报错退出 1）、`source_mode: host`（主机目录备份完全不碰 adb、归档可校验），旧的 `--list-tree`/`--clean-*` 报错并提示新写法、`clean host-cache`、Android/data 路径、输出目标预检（路径某段是文件、父目录不可写时报错且不产生任何 adb 调用） |
