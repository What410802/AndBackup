...
随仓库模板（`backup-android.example.yaml`）默认采用 `device-python`（推荐）；此时
`download_device_python` 缺省即为 `true`，首跑联网下载并缓存解释器。下表“内置默认”指该键未
出现在配置中时的值；“模板”列为示例配置的取值。
...
2. 将 YAML 中的 `host` 填为设备显示的连接地址 `设备IP:连接端口`（只填 `IP` 也可以，
   `adb` 会用默认端口并自动连接）。
...
Android 每次显示新的无线连接端口时，只需更新 YAML 的 `host` 行；USB 配置将 `host`
留空即可（只有 `host` 非空才会触发 `adb connect`）。
...
临时文件保护。开工前还会用该 `.partial.*` 占位文件预检输出目标：目标已是目录、不是普通文件、
路径某段是文件、父目录不可写（Windows 上还有目标只读/被占用）都会在**任何设备操作之前**失败，
交互终端可改输新路径；万一最后改名失败，校验过的归档保留在该 `.partial.*` 里。
...
并使用 `rmdir` 而非 `rm -rf`（枚举不完整时只删普通文件与符号链接）。`--prune-dry-run`
只列出将删除的内容；交互终端会打印计划并询问一次；备份或校验失败时不删除任何源条目。

目标文件（含后缀）已存在时不会静默覆盖：交互终端会先问“覆盖它吗？[y/N]”（默认不覆盖，
答 `n`/回车可改输另一个路径），非交互环境直接报错退出并提示加 `--force`，避免无人值守时
把上一次的备份覆盖掉。加 `-f`/`--force`（或配置 `force: true`）即跳过询问直接覆写；它只
决定“是否询问”，不会绕过目标本身写不进去的情况。
